...
# Workout End Points
@router.get("/users/{user_uuid:uuid}/workouts", response_model=WorkoutListResponse, status_code=status.HTTP_200_OK)
async def get_workouts(user_uuid: UUID, session: Session = Depends(get_db)) -> WorkoutListResponse:
    ...
    workouts: list[Workout] = session.exec(select(Workout).where(Workout.user_uuid == user_uuid)).all()
...
@router.get("/users/{user_uuid:uuid}/workouts/{workout_uuid:uuid}", response_model=WorkoutResponse, status_code=status.HTTP_200_OK)
async def get_workout(user_uuid: UUID, workout_uuid: UUID, session: Session = Depends(get_db)) -> WorkoutResponse:
    if not session.exec(select(User).where(User.uuid == user_uuid)).first():
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"User UUID: {user_uuid} not found.")
...
@router.post("/users/{user_uuid:uuid}/workouts/", response_model=WorkoutResponse, status_code=status.HTTP_201_CREATED)
async def create_workout(user_uuid: UUID, workout_request: WorkoutCreateReq, session: Session = Depends(get_db)) -> WorkoutResponse:
    if not session.exec(select(User).where(User.uuid == user_uuid)).first():
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"User UUID: {user_uuid} not found.")
    workout = Workout.model_validate(workout_request.model_dump(), update={"user_uuid": user_uuid})
    session.add(workout)
    session.commit()
...
@router.put("/users/{user_uuid:uuid}/workouts/{workout_uuid:uuid}", response_model=WorkoutResponse, status_code=status.HTTP_200_OK)
async def update_workout(user_uuid: UUID, workout_uuid: UUID, workout_request: WorkoutCreateReq, session: Session = Depends(get_db)) -> WorkoutResponse:
    if not session.exec(select(User).where(User.uuid == user_uuid)).first():
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"User UUID: {user_uuid} not found.")
    workout = session.exec(select(Workout).where(Workout.uuid == workout_uuid)).first()
    if not workout:
...
@router.patch("/users/{user_uuid:uuid}/workouts/{workout_uuid:uuid}", response_model=WorkoutResponse, status_code=status.HTTP_200_OK)
async def patch_workout(user_uuid: UUID, workout_uuid: UUID, workout_request: WorkoutPatchReq, session: Session = Depends(get_db)) -> WorkoutResponse:
    if not session.exec(select(User).where(User.uuid == user_uuid)).first():
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"User UUID: {user_uuid} not found.")
    workout = session.exec(select(Workout).where(Workout.uuid == workout_uuid)).first()
    if not workout:
...
@router.delete("/users/{user_uuid}/workouts/{workout_uuid}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_workout(user_uuid: UUID, workout_uuid: UUID, session: Session = Depends(get_db)):
    if not session.exec(select(User).where(User.uuid == user_uuid)).first():
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"User UUID: {user_uuid} not found.")
    workout = session.exec(select(Workout).where(Workout.uuid == workout_uuid)).first()
    if not workout:
...
@router.post("/users/{user_uuid}/workouts/{workout_uuid}/workout_exercises", response_model=WorkoutResponse, status_code=status.HTTP_200_OK)
async def link_workout_exercise_to_workout(user_uuid: UUID, workout_uuid: UUID, workout_add_workout_exercise_request: WorkoutAddWorkoutExerciseReq, session: Session = Depends(get_db)) -> WorkoutResponse:
...
@router.delete("/users/{user_uuid}/workouts/{workout_uuid}/exercises/{workout_exercise_uuid}", status_code=status.HTTP_204_NO_CONTENT)
